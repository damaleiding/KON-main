...
- `references/`：跨项目参考资料入口。项目专属参考资产优先放进对应项目的 `refs/`。
...
- `_ledger/`：生产账本、prompt 索引、审稿决策、版本流转和 manifest 等事实记录。
...
- 人类项目纲要应避免堆砌脚本细节，重点说明“这是干什么的、有什么资产、从哪里开始、当前状态、下一步建议”。
- 项目的目标、作用、目录结构、关键资产状态或工作流发生明显变化时，必须同步更新 `<project-name>_project-brief.html`。
